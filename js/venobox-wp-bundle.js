(function () {
   ...
   */
   ...
   /**
    * VenoBox initialization for WordPress
    */
   const VenoboxWP = (function() {

       // VENOBOX is a global object provided by WordPress wp_localize_script
       ...
       function initGalleries() {
           document.querySelectorAll('.wp-block-woocommerce-product-gallery, .wc-block-product-gallery, .woocommerce-product-gallery');

           // Set galleries to have unique data-gall sets
           const galleries = document.querySelectorAll('div[id^="gallery"], .gallery-row, .wp-block-gallery, .woocommerce-product-gallery, .wp-block-woocommerce-product-gallery, .wc-block-product-gallery');
       ...
       // Default settings
       let vboxBatchCounter = 0;
       let initialLoadFinished = false;

       function defaultSettings() {
           const uninitialized = document.querySelectorAll('.venobox:not(.vbox-initialized)');
           if (uninitialized.length > 0) {
               vboxBatchCounter++;
               const batchClass = 'vbox-initialized-' + vboxBatchCounter;
               uninitialized.forEach(function(el) {
                   el.classList.add('vbox-initialized', batchClass);
               });
               new VenoBox({
                   selector: '.' + batchClass,
       ...
       function updateImgLink(img) {
           if (img.closest('.wc-block-product-gallery-thumbnails, .flex-control-thumbs')) {
               return;
           }
           ...
           if (!originalUrl) {
               ...
               if (src) {
                   originalUrl = src.replace(/-[0-9]+x[0-9]+(\.[a-zA-Z0-9]+)$/, '$1');
               }
           }
           ...
           if (originalUrl) {
               ...
               if (!link.classList.contains('venobox')) {
                   link.classList.add('venobox');
                   isNew = true;
               }
               if (!link.hasAttribute('data-gall')) {
                   link.dataset.gall = 'gallery';
               }
               
               if (isNew && initialLoadFinished) {
                   initNewElements();
               }
           }
       }
       ...
       function initWooCommerceBlocks() {
           const wooGalleries = document.querySelectorAll('.wp-block-woocommerce-product-gallery, .wc-block-product-gallery, .woocommerce-product-gallery');
           
           wooGalleries.forEach(function(gallery) {
               // Process existing images
               const imgs = gallery.querySelectorAll('img');
               imgs.forEach(updateImgLink);

               // Setup observer specifically for this gallery wrapper if not already observed
               if (typeof MutationObserver !== 'undefined' && !gallery.dataset.vboxObserved) {
                   gallery.dataset.vboxObserved = '1';
